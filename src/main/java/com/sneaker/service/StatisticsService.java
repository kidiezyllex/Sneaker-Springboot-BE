...
public class StatisticsService {
        ...
        public Map<String, Object> getStatistics(String type, LocalDateTime startDate, LocalDateTime endDate) {
                List<Order> orders = orderRepository.findAll().stream()
                                ...
                                .filter(o -> {
                                        if (startDate != null && o.getCreatedAt().isBefore(startDate))
                                                return false;
                                        if (endDate != null && o.getCreatedAt().isAfter(endDate))
                                                return false;
                                        return true;
                                })
                                .toList();
                ...
                long newCustomers = accountRepository.findAll().stream()
                                ...
                                .filter(a -> {
                                        if (startDate != null && a.getCreatedAt().isBefore(startDate))
                                                return false;
                                        if (endDate != null && a.getCreatedAt().isAfter(endDate))
                                                return false;
                                        return true;
                                })
                                .count();
        ...
        public List<Map<String, Object>> getRevenueReport(LocalDateTime startDate, LocalDateTime endDate,
                ...
                DateTimeFormatter formatter = groupBy.equals("month") ? DateTimeFormatter.ofPattern("yyyy-MM")
        ...
        public List<Map<String, Object>> getTopProducts(LocalDateTime startDate, LocalDateTime endDate, int limit) {
                ...
                return productStats.values().stream()
                                .sorted((a, b) -> ((Integer) b.get("totalSold"))
                                                .compareTo((Integer) a.get("totalSold")))
                                .limit(limit)
                                .toList();
        }
