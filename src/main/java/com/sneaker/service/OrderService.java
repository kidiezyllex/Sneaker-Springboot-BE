...
public class OrderService {
        ...
        for (int i = 0; i < request.getItems().size(); i++) {
            ...
            Product product = productRepository.findById(productId)
                    .orElseThrow(() -> new RuntimeException(
                            "Không tìm thấy sản phẩm với ID: " + productId + " trong item[" + itemIndex + "]"));
    ...
    public Page<Order> getOrders(Integer customerId, Order.OrderStatus orderStatus,
        ...
        return orderRepository.findWithFilters(customerId, orderStatus, paymentStatus, startDate, endDate, search,
                pageable);
    }

    public Page<Order> getMyOrders(Integer customerId, Order.OrderStatus orderStatus, Pageable pageable) {
        if (orderStatus != null) {
            return orderRepository.findByCustomerIdAndOrderStatus(customerId, orderStatus, pageable);
        }
    ...
    public Order updateOrder(Integer id, Order orderDetails) {
        ...
        if (orderDetails.getShippingPhoneNumber() != null)
            order.setShippingPhoneNumber(orderDetails.getShippingPhoneNumber());
        if (orderDetails.getShippingProvinceId() != null)
            order.setShippingProvinceId(orderDetails.getShippingProvinceId());
        if (orderDetails.getShippingDistrictId() != null)
            order.setShippingDistrictId(orderDetails.getShippingDistrictId());
        if (orderDetails.getShippingWardId() != null)
            order.setShippingWardId(orderDetails.getShippingWardId());
        if (orderDetails.getShippingSpecificAddress() != null)
            order.setShippingSpecificAddress(orderDetails.getShippingSpecificAddress());
